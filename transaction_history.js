...
const phoneNumber = readlineSync.question("Masukan Nomor Hp : ");
...
const transactionHistory = (access_token, uuid, uniqid) =>
  new Promise((resolve, reject) => {
...
(async () => {
  ...
  const sendOtp = await sendOTP(uuid, uniqueid);
  ...
  const otp = await readlineSync.question("Masukan kode OTP : ");
  const verifotp = await loginVerify(token, otp, uuid, uniqueid);
  const access_token = JSON.parse(getAccessToken).data.access_token;
  const cek = await transactionHistory(access_token, uuid, uniqueid);
